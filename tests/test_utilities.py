...
def test_sph2cart():
    ...
    np.testing.assert_allclose(np.array(utl.sph2cart(np.math.pi, 0, 0)),
                               np.zeros(3))
    np.testing.assert_almost_equal(np.array(utl.sph2cart(np.math.pi / 2.0, 0, 1)),
                                   np.array((1, 0, 0)))
    np.testing.assert_almost_equal(np.array(utl.sph2cart(np.math.pi / 2.0, np.math.pi / 2.0, 1)),
                                   np.array((0, 1, 0)))
    np.testing.assert_almost_equal(np.array((utl.sph2cart(np.math.pi, np.math.pi / 2.0, 1))),
                                   np.array((0, 0, -1)))
